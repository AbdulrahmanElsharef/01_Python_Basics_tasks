...
class Items():  # parents class
	...
	def __init__(self):
		# welcome message
		print(f"\n*** welcom to {Items.__name__} programm ***".title())
		self.All_Users = {"abdo.elsharef": {
			"user_pass": "abdo123456789", "user_depart": "admin"}}
		self.All_items = {}
	...
	def Sighn_Up(self):
		...
		input:
		(string :user name & user password & user department) #capital or small or numbers or special
		...
		while True:
			...
			if item_seiral in self.All_items.keys():
				if item_account.lower() == "stock":
					...
					print(
						f"stok of item ({item_seiral}) = ({cur_stock} items)".title())
					print("*****"*5)
					...
					print(
						f"cost of Purshaing  item ({item_seiral}) = items ({item_buy}) * cost price ({item_cost_price}) = ({item_cost} $)".title())
					print(
						f"current stock of item is ({stock_item} items)".title())
					print("*****"*5)
					...
					print(
						f"total sales of item ({item_seiral}) = sales item ({sales_item}) * sales price ({item_sell_price}) = ({item_sales} $)".title())
					print(
						f"current stock of item is ({stock_item} items)".title())
					print("*****"*5)
					...
					print(
						f"profit of item ({item_seiral}) = total sales ({item_sales}) - total cost ({item_cost}) = ({total_profit} $)".title())
					print("*****"*5)
